...
class ProfileRestAPIClass {
    ...
    class func getProfileGradeList(completion: (data: JSON?, error: HTTPErrorType?) -> (), failure: (error: NetworkErrorType?) -> ()) {
        guard let fullPath = UrlMakerSingleton.sharedInstance.listGradeProfileUrl() else {
            return
        }
